...
def load_coco_json(json_file, image_root, metadata, dataset_name):
    ...
    split_name='misc210k_cocosplit' if 'unify' in dataset_name  or 'misc' in dataset_name  else 'cocosplit'
    if is_shots:
        imgid2info = {}
        shot = dataset_name.split('_')[-2].split('shot')[0]
        seed = int(dataset_name.split('_seed')[-1])
        split_dir = os.path.join('datasets', split_name, 'seed{}'.format(seed))
