...
def default_argument_parser():
    ...
    parser.add_argument("--port", type=int, default=2 ** 10 + hash(os.getuid()) % 2 ** 15,
                        help="the rank of this machine")
    # port =  2 ** 10 + hash(os.getuid()) % 2 ** 15 #14
    #port = 49523
    # parser.add_argument("--dist-url", default="tcp://127.0.0.1:{}".format(port))
    parser.add_argument("--dist-url", default="tcp://127.0.0.1:")
...
def default_setup(cfg, args):
    ...
    setup_logger(output_dir, distributed_rank=rank, name="dcfs")
...
class DefaultTrainer(SimpleTrainer):
        ...
        if comm.is_main_process():
            ret.append(
                hooks.PeriodicCheckpointer(
                    self.checkpointer, cfg.SOLVER.CHECKPOINT_PERIOD,max_to_keep=cfg.SOLVER.MAX_TO_KEEP
                )
            )
    ...
    def test(cls, cfg, model, evaluators=None):
        ...
        #     logger.info(f"Results for {dataset_name} in csv format:")
