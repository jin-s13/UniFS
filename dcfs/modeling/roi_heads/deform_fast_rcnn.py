...
class DeformFastRCNNOutputs(object):
    """
    ...
    def __init__(
        ...
        self.pred_class_logits_list = pred_class_logits_list
        self.pred_class_logits = pred_class_logits_list[0]
    ...
    def dc_loss(self,pred_class_logits):
        """
        ...
        num_instances = pred_class_logits.shape[0]
        ...
        pred_logits = pred_class_logits * knonw_class_mask
    ...
    def ciou_loss(self):
        ...
        Returns:
            scalar Tensor
        """
        gt_proposal_deltas =  self.gt_boxes.tensor
        box_dim = gt_proposal_deltas.size(1)  # 4 or 5
        cls_agnostic_bbox_reg = self.pred_proposal_deltas.size(1) == box_dim
        device = self.pred_proposal_deltas.device
    ...
    def losses(self):
        ...
        if self.box_class_loss_type=="DC":
            return {
                "loss_cls1": self.dc_loss(self.pred_class_logits_list[0]),
                "loss_cls2": self.dc_loss(self.pred_class_logits_list[1]),
                # "loss_cls_mean": self.dc_loss(self.pred_class_logits),
                "loss_box_reg": self.ciou_loss(),
            }
        else:
            self._log_accuracy()
            return {
                "loss_cls1": F.cross_entropy(self.pred_class_logits_list[0], self.gt_classes, reduction="mean"),
                "loss_cls2": F.cross_entropy(self.pred_class_logits_list[1], self.gt_classes, reduction="mean"),
                # "loss_cls_mean": F.cross_entropy(self.pred_class_logits, self.gt_classes, reduction="mean"),
                "loss_box_reg": self.ciou_loss(),
            }
    ...
    def predict_boxes(self):
        ...
        #     .reshape(-1, B),
        # )
        boxes = self.pred_proposal_deltas.view(num_pred * K, B)
...
class DeformFastRCNNOutputLayers(nn.Module):
    """
    ...
    def __init__(
        self, cfg, input_size,num_classes, cls_agnostic_bbox_reg, box_dim=4,input_size_cls=2048
    ):
        """
        Args:
            cfg: config
        ...
        """
        super(DeformFastRCNNOutputLayers, self).__init__()
        ...
        # background class
        # (hence + 1)
        self.cls_score = nn.Linear(input_size_cls, num_classes + 1)
        self.cls_score1 = nn.Linear(input_size_cls, num_classes + 1)
        num_bbox_reg_classes = 1 if cls_agnostic_bbox_reg else num_classes
        self.bbox_pred = nn.Linear(input_size, num_bbox_reg_classes * box_dim)

        nn.init.normal_(self.cls_score.weight, std=0.01)
        nn.init.normal_(self.cls_score1.weight, std=0.01)
        nn.init.normal_(self.bbox_pred.weight, std=0.001)
        for l in [self.cls_score,self.cls_score1, self.bbox_pred]:
