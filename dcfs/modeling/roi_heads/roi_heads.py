...
class Res5ROIHeads(ROIHeads):
    ...
    def __init__(self, cfg, input_shape):
        ...
        if self.keypoint_on:
            if pooler_type:
                shape = ShapeSpec(
                    channels=out_channels, width=pooler_resolution,
                    height=pooler_resolution
                )
            else:
                shape = {f: input_shape[f] for f in out_channels}
            self.keypoint_head = build_keypoint_head(cfg, shape)
    ...
    def forward(self, images, features, proposals, targets=None, fs_class=None):
        ...
        if self.training:
            ...
            if self.keypoint_on:
                losses.update(self.keypoint_head(mask_features, proposals))
            return [], losses
        else:
    ...
    def forward_with_given_boxes(self, features, instances):
        ...
        if self.keypoint_on:
            instances= self.keypoint_head(x, instances)
