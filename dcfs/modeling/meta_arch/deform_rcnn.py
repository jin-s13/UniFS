...
__all__ = ["DeformFasterRCNN"]


@META_ARCH_REGISTRY.register()
class DeformFasterRCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.MODEL.ROI_HEADS.FREEZE_FEAT:
            for p in self.roi_heads.res5.parameters():
                p.requires_grad = False
            # for p in self.roi_heads.down_sample.parameters():
            #     p.requires_grad = False
    ...
    def _forward_once_(self, batched_inputs, gt_instances=None, fs_class=None):
        ...
        batch_input_shape = tuple(images.tensor[0].size()[-2:])
        for img_meta in batched_inputs:
        ...
        results, detector_losses = self.roi_heads(batched_inputs, features_de_rcnn, proposals, gt_instances, fs_class)
